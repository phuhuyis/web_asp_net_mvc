...
$('.add-cart-home').off('click').on('click', async function (e) {
    ...
    var btn = $(this);
    var id = btn.attr("data-id");
    $('.add-cart-modal').attr('data-id', id);
    $("#select-color").modal("show");
});
...
async function update(product, color, quantity) {
    return $.ajax({
        url: "/api/cart",
        dataType: "json",
        data: {
...
async function del(product, color, quantity) {
    return $.ajax({
        url: "/api/cart",
        dataType: "json",
        data: {
...
async function pay(id, quantity, color) {
    var name = $("#name").val();
    var address = $("#address").val();
    var phone = $("#phone").val();
    var payment = $("#payment").val();
...
$('.del-cart').off('click').on('click', async function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var rsDel = await del(btn.data('product'), btn.attr('data-color'), btn.attr('data-quantity'));
            if (rsDel) {
                document.location.href = document.location.href;
            }
        }
...
$('.quantity-input').on('change', async function (e) {
    ...
    if (input.val() <= 0) {
        ...
        var rsUpdate = await update(input.data('product'), input.attr("data-color"), input.val());
        if (rsUpdate) {
            document.location.href = document.location.href;
        }
    }
...
$(document).on('click', '.number-spinner-color button', function () {
    var btn = $(this),
        oldValue = btn.closest('.number-spinner-color').find('input').val().trim(),
        newVal = oldValue;
    ...
    btn.closest('.number-spinner-color').find('input').val(newVal);
